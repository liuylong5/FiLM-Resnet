...
/* $Header: /opt/home/shunliu/src/cmaq/4.7.1/models/CCTM/src/par/par/distr_env.c,v 1.1.1.1 2010/06/14 16:03:06 sjr Exp $  */
...
extern void distr_env_ (int *myid_p, int *numprocs_p)
{
   ...
   if (myid != 0)
   {
      ...
      if (myid == 1)
      {
         printf ("total_size_0 is: %d \n", total_size_0);
      printf ("temp_buf is: %p \n", temp_buf);}
      ...
      if (strlen(curr_ptr) <= CURR_STR_SIZE)
       {
            ...
            if (myid == 1)
            {
               printf ("strlen of curr_ptr is: %d \n", strlen(curr_ptr));
            printf ("strlen of curr_str is: %d \n", strlen(curr_str));
            printf ("The current environmental value pair is: %s \n", curr_str);
            }
       }
       else
       {
...
*/
      printf ("kkkkkkkkkkkkk5555 \n");
   MPI_Barrier (MPI_COMM_WORLD);
      printf ("kkkkkkkkkkkkk666 \n");
