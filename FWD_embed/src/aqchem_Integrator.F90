...
! 
! File                 : aqchem_Integrator.F90
! Time                 : Tue Mar  4 17:13:24 2014
! Working directory    : /home/kfahey/kpp-2.2.3/ASSUMPTION_TEST/CLEAN_INIT/TESTS/CMAQ_BASE_022414
...
!slz   USE UTILIO_DEFN  !kf  uncomment for CMAQ
     
   REAL(kind=dp), INTENT(IN) :: T, H
   INTEGER, INTENT(IN)  :: Code
   INTEGER, INTENT(OUT) :: IERR

   CHARACTER( 120 ) :: XMSG = ' '  ! Exit status message  !kf
   INTEGER :: XSTAT3 !slz
   CHARACTER( 16 ), SAVE :: PNAME = 'AQCHEM'             ! driver program name  !kf   
...
!kf   END SELECT
    ...
    CASE (-1)    
      PRINT * , '--> Improper value for maximal no of steps'
       XMSG = 'Ros ERROR: Improper value for maximal no of steps'  !kf
    CASE (-2)    
      PRINT * , '--> Selected Rosenbrock method not implemented'
      XMSG = 'Ros ERROR: Selected Rosenbrock method not implemented'  !kf
    CASE (-3)    
      PRINT * , '--> Hmin/Hmax/Hstart must be positive'
      XMSG = 'Ros ERROR: Hmin/Hmax/Hstart must be positive'  !kf
    CASE (-4)    
      PRINT * , '--> FacMin/FacMax/FacRej must be positive'
      XMSG = 'Ros ERROR: FacMin/FacMax/FacRej must be positive'  !kf
    CASE (-5) 
      PRINT * , '--> Improper tolerance values'
      XMSG = 'Ros ERROR: Improper tolerance values'  !kf
    CASE (-6) 
      PRINT * , '--> No of steps exceeds maximum bound'
      XMSG = 'Ros ERROR: No of steps exceeds maximum bound'  !kf
    CASE (-7) 
      PRINT * , '--> Step size too small: T + 10*H = T', &
            ' or H < Roundoff'
      XMSG = 'Ros ERROR: Step size too small:'  !kf    
    CASE (-8)    
      PRINT * , '--> Matrix is repeatedly singular'
      XMSG = 'Ros ERROR: Matrix is repeatedly singular'  !kf
    CASE DEFAULT
      PRINT *, 'Unknown Error code: ', Code
      XMSG = 'Ros ERROR: Unknown Error code: '  !kf
   END SELECT

    CALL M3EXIT ( PNAME, JDATEKPP, JTIMEKPP, XMSG, XSTAT3 ) ! kf uncomment for CMAQ   
   PRINT *, "T=", T, "and H=", H
   
!   STOP !kf comment for CMAQ
...
TimeLoop: DO WHILE ( ABS(T-Tend) >= ROUNDOFF ) 
...
!kf   
!        IF (Direction > 0) THEN
!        TESTCOND = T-Tend+Roundoff
!	ELSEIF (Direction < 0) THEN
!	TESTCOND = Tend-T+Roundoff
!	ENDIF   
...
!kf
   IF ( IERR .NE. 1) THEN
          XMSG = 'AQCHEM INTEGRATION UNSUCCESSFUL'
        CALL M3EXIT ( PNAME, JDATEKPP, JTIMEKPP, XMSG, XSTAT3 )  ! uncomment for CMAQ
        END IF
!kf	
