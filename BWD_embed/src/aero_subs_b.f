...
!slz          CALL GETCOAGS_ADJ(lamda, kfmatac, kfmatacb, kfmat, kfmatb, kfmac,
!slz     +              kfmacb, knc, dg_d(1), dg_db(1), dg_d(2), dg_db(2), sg_d(1)
!slz     +              , sg_d(2), xxl_d(1), xxl_db(1), xxl_d(2), xxl_db(2), batat
!slz     +              (2), batatb(2), batat(1), batatb(1), bacac(2), bacacb(2),
!slz     +              bacac(1), bacacb(1), batac(2), batacb(2), bacat(2), bacatb
!slz     +              (2), batac(1), batacb(1), c3ij, c3ijb)
          DO n=n_mode,1,-1
...
!slz          CALL INTERCOAG_GH_ADJ(lamda, kfmatac, kfmatacb, knc, aeromode_diam(1)
!slz     +                  , aeromode_diamb(1), aeromode_diam(2), aeromode_diamb(
!slz     +                  2), aeromode_sdev(1), aeromode_sdevb(1), aeromode_sdev
!slz     +                  (2), aeromode_sdevb(2), batac(2), batacb(2), bacat(2)
!slz     +                  , bacatb(2), batac(1), batacb(1), c3ij, c3ijb)
...
!slz          CALL INTRACOAG_GH_ADJ(lamda, kfmac, kfmacb, knc, aeromode_diam(2),
!slz     +                  aeromode_diamb(2), aeromode_sdev(2), aeromode_sdevb(2)
!slz     +                  , bacac(2), bacacb(2), bacac(1), bacacb(1))
...
!slz          CALL INTRACOAG_GH_ADJ(lamda, kfmat, kfmatb, knc, aeromode_diam(1),
!slz     +                  aeromode_diamb(1), aeromode_sdev(1), aeromode_sdevb(1)
!slz     +                  , batat(2), batatb(2), batat(1), batatb(1))
        END IF
...
!slz           CALL HETCHEM_ADJ(gamma_n2o5, dt)
...
!
            IF (trustiso) THEN
              CALL PUSHREAL8ARRAY(j, 3)
              CALL PUSHREAL8ARRAY(gas, 3)
...
C         GAS( 3 ) = GCLR8  * ( 1.0D-6 / dble(PRECURSOR_MW( HCL_IDX )) )
        ...
        trustcl = .true.
        IF (wi(1) + wi(5) .LT. 1.0d-20) THEN !slzdbg -20
          trustcl = .false.
        ELSE IF (wi(5) .LT. 1.0d-10) THEN !slzdbg -10
          trustcl = .false.
        END IF
!slzdbg        trustcl = .false. !slzdbg
     ...
     +                scasi, other, TrustIso)
      if (( AIRTEMP .le. 250.0 )
     +       .or.( AIRPRS .le. 20000.0 ) ) THEN
         TrustIso = .false.
      endif
!slzdbg     trustiso = .false. !slzdbg
...
!
          ...
          if(drym3.lt.1.d-30) then
           call pushinteger4(1)
           drym3=1.d-30 !slzdbg
           write (*,*) "slzdbg-drym3-reset"
          else
           call pushinteger4(0)
          endif
          drym20 = wetm2*(drym3/wetm3)**d_twothirds
...
100       cinorgb(isp, imode) = cinorgb(isp, imode) + cfinalb(isp, imode)
     ...
     +        INT(d_twothirds))) THEN
          ...
          drym3b = temp3b9
          call popinteger4(branch)
          if(branch.eq.1) drym3b=0.d0 !slzdbg
...
160     CALL POPREAL8(wi(5))
     ...
     +        aerospc_mw(anh4_idx)
            ...
            IF (branch .LT. 1) THEN
              rate = condso4(imode)
              CALL POPREAL8ARRAY(gas, 3)
              CALL POPREAL8ARRAY(j, 3)
...
!	  gclr8b=gclr8b+gasb(3)* ( 1.0D-6 / DBLE(PRECURSOR_MW( HCL_IDX )) )
     ...
     +                fconc_so4b(3, :))
!slz              CALL HCOND3_ADJ(am0(3), am0b(3), am1(3), am1b(3), am2(3), am2b(3)
!slz     +                , dv_so4, alphsulf, cbar_so4, fconc_so4(3, :),
!slz     +                fconc_so4b(3, :))
              CALL POPREAL8(am2(3))
...
!slz        CALL NEWPART3_ADJ(airrh, airtemp, xh2so4, xh2so4b, so4rate, so4rateb,
!slz     +            dndt, dndtb, dmdt_so4, dmdt_so4b, dm2dt, dm2dtb )
