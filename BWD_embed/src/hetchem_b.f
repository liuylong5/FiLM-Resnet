...
!
        ...
        ELSE
          ...
          precursor_concb(hno3_idx) = 0.0
        END IF
        tempb7 = faerno2*0.5*dfhono*ghonob
        tempb6 = faerno2*0.5*dfhno3*ghno3b
        tempb8 = faern2o5*2.0*dfhno3*ghno3b
        CALL POPREAL4(gno2)
        expdt_no2b = gno2*gno2b - gno2*tempb7 - gno2*tempb6
        gno2b = (1.0-expdt_no2)*tempb6 + (1.0-expdt_no2)*tempb7 + expdt_no2*
     +   gno2b
        CALL POPREAL4(gn2o5)
        expdt_n2o5b = gn2o5*gn2o5b - gn2o5*tempb8
        gn2o5b = (1.0-expdt_n2o5)*tempb8 + expdt_n2o5*gn2o5b
        ...
        tempb1 = cbar*pi*kn2o5b
        xxf_atb = gamma*tempb1
        xxf_acb = gamma*tempb1
        temp14 = 0.5*cbar*de_ac_wet*gamma/diff_n2o5 + 4.0
        tempb2 = -(cbar*0.5*wet_m2_j*xxf_acb/(temp14**2*diff_n2o5))
        de_ac_wetb = gamma*tempb2
        temp13 = 0.5*cbar*de_at_wet*gamma/diff_n2o5 + 4.0
        tempb3 = -(cbar*0.5*wet_m2_i*xxf_atb/(temp13**2*diff_n2o5))
        de_at_wetb = gamma*tempb3
        ...
        IF (temp5 .EQ. 0.0) THEN
          tempb0 = 0.0
        ELSE
          tempb0 = aeromode_diam(2)*dg_ac_wetb/(2.0*temp6*old_m2_j)
        END IF
        ...
        IF (temp3 .EQ. 0.0) THEN
          tempb = 0.0
        ELSE
          tempb = aeromode_diam(1)*dg_at_wetb/(2.0*temp4*old_m2_i)
        END IF
        wet_m2_ib = xxf_atb/temp13 + tempb + pi*totsurfab
        wet_m2_jb = xxf_acb/temp14 + tempb0 + pi*totsurfab
        gammab = de_ac_wet*tempb2 + de_at_wet*tempb3 + (xxf_at+xxf_ac)*tempb1
        aeromode_sdevb(2) = aeromode_sdevb(2) + 2.0*temp11*1.5*EXP(temp10)*
     +   dg_ac_wet*EXP(aeromode_sdev(2))*de_ac_wetb/temp12
        aeromode_sdevb(1) = aeromode_sdevb(1) + 2.0*temp8*1.5*EXP(temp7)*
     +   dg_at_wet*EXP(aeromode_sdev(1))*de_at_wetb/temp9
        aeromode_diamb(2) = aeromode_diamb(2) + temp6*dg_ac_wetb
        old_m2_jb = temp1**temp2*wet_m2_jb - temp5*tempb0
        aeromode_diamb(1) = aeromode_diamb(1) + temp4*dg_at_wetb
        old_m2_ib = temp**temp0*wet_m2_ib - temp3*tempb
        IF (temp1 .LE. 0.0 .AND. (temp2 .EQ. 0.0 .OR. temp2 .NE. INT(temp2)))
     + THEN
          tempb4 = 0.0
        ELSE
          tempb4 = temp2*temp1**(temp2-1)*old_m2_j*wet_m2_jb/old_m3_j
        END IF
        wet_m3_jb = tempb4
        old_m3_jb = wet_m3_jb - temp1*tempb4
        IF (temp .LE. 0.0 .AND. (temp0 .EQ. 0.0 .OR. temp0 .NE. INT(temp0)))
     + THEN
          tempb5 = 0.0
        ELSE
          tempb5 = temp0*temp**(temp0-1)*old_m2_i*wet_m2_ib/old_m3_i
        END IF
        wet_m3_ib = tempb5
        old_m3_ib = wet_m3_ib - temp*tempb5
